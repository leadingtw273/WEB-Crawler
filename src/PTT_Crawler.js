...
// 取得單頁資料
const getPageDataList = (searchRegExp, link) => new Promise((resolve) => {
  const opt = {
    url: link,
    headers: {
      Cookie: 'over18=1',
  ...
  request(opt, (err, res, body) => {
    ...
    // 抓取文章列表
    let list = $('.r-ent a').map((index, obj) => domDataSet($(obj))).get();
...
// 取得最新頁面index
const getLastPageIndex = link => new Promise((resolve) => {
  const opt = {
    url: link,
    headers: {
      Cookie: 'over18=1',
  ...
  request(opt, (err, res, body) => {
    ...
    // 抓取最新頁數
    const prePageUrl = $('.btn-group-paging a').filter((i, el) => $(el).text() === '‹ 上頁').attr('href');
    const lastUrlIndex = parseInt(prePageUrl.split('/').pop().slice(5, -5), 10) + 1;
...
// 主要執行
const main = config => getLastPageIndex(config.URL).then(async (val) => {
  let data = [];
  let dataList = [];
  let targetUrl = '';
  ...
  // 輸出至指定檔案
  try {
    fs.writeFileSync('./lib/RegExpTest/data.json', JSON.stringify({ dataList: dd.map(x => x.title) }));
  } catch (err) {
    console.log(err);
  }
